      ...
      <table class="min-w-full bg-gray-600 rounded-lg overflow-hidden shadow-lg">
        <thead class="bg-gray-800 text-white">
          <tr>
            ...
            <th class="py-2 px-4">Lap Time</th>
            <th class="py-2 px-4">Session Date</th>
            <th class="py-2 px-4">Session Time</th>
            <th class="py-2 px-4">Driver Name</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(entry, index) in laptimes" :key="entry.id" class="border-b border-gray-200">
            <td class="py-2 px-4 text-center">{{ index + 1 }}</td>
            <td class="py-2 px-4 text-center">{{ formatLapTime(entry.laptimeMinutes, entry.laptimeSeconds, entry.laptimeMilliseconds) }}</td>
            <td class="py-2 px-4 text-center">{{ entry.sessionDate }}</td>
            <td class="py-2 px-4 text-center">{{ entry.sessionTime }}</td>
            <td class="py-2 px-4 text-center">{{ entry.driverFirstName + ' ' + entry.driverLastName }}</td>
          </tr>
        </tbody>
      </table>
    </section>
